...
**This is the canonical brief for any AI assistant working on this repo** —
Claude Code, OpenAI's coding agent, Cursor, Copilot, or anything else. The
guidance is the same whoever is reading. `CLAUDE.md` is a stub that points
here so Claude Code's auto-loading finds it; keep the content in this file
only, so the two can never drift apart.
