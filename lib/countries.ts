/**
 * Full ISO 3166-1 alpha-2 country list so any origin → destination lane can
 * be selected. FREQUENT_LANES keeps the launch-focus countries grouped at the
 * top of the pickers — they're where our reference data (trade programs,
 * de minimis, VAT placeholders) is deepest, but classification itself works
 * for every lane: HS codes are harmonized at 6 digits worldwide.
 */
...
/** Launch-focus lanes, shown as the top group in country pickers. */
export const FREQUENT_LANES: string[] = [
  "US", "DE", "GB", "TR", "FR", "NL", "IT", "ES", "CN", "IN", "PL", "BE",
];

/** All countries as sorted [code, name] entries (by display name). */
export const ALL_COUNTRY_OPTIONS: [string, string][] = Object.entries(
  ALL_COUNTRIES,
